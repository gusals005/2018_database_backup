...
*/
int internal_order = 4;   // order of internal_node 249
int leaf_order = 4;        // order of leaf node 32
...
void print_leaves(int table_id) {      // leaves 를 출력하는 문
    ...
    while (true) {      // 내꺼 출력하고 내 sibling들 출력.
        ...
        if ( c->special_page_offset != -1) {       // order - 1 index에는 다음 leaf의 주소가 들어가 있음.
            printf(" | ");
            c->page_type = 2;
            buffer_get_page(table_id,c->special_page_offset/PAGE_SIZE,c);
            buffer_put_page(table_id, c->pagenum, c);
...
void find_and_print( int table_id , int64_t key) {   // key를 넘기고, verbose를 넘기는데, p 이면 true, f이면 false
    ...
    buffer_put_page(table_id, 0, header);
    int i;
    int64_t * r = find( table_id, key);      // key 가 존재하는지 find를 하고,
...
int64_t * find(int table_id, int64_t find_key ) {   // find, key와 verbose를 받음(verbose가 true 면 path를 출력 ,false 면 그수만 찾기)
...
int  insert( int table_id, int64_t key, int64_t * value ) {      // insert를 하자
    ...
    if (find(table_id , key) != NULL)     //우선, 그 값이 있는지 찾아.NULL이 아니라면 -> return 을 시켜라 ( NULL이 아니라는 건 노드가 있다는것) ),
...
int erase(int table_id, int64_t key) {       // root 를 입력받고 key를 입력받음
    ...
    key_record = find(table_id, key);        // find를 할 건데, root,key,false를 넣을 거.   
