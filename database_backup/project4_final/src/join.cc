...
unordered_multimap<int64_t, vector<int64_t > > u_map[NUM_THREADS];
...
void* input_to_hash(void* thread_num){
    ...
    while(1){
        ...
        if(tnum ==0 && !left_table_hash_or_right_table_hash)
        ...
        if(left_table_hash_or_right_table_hash) // now is hashing left_table
        {
            ...
            for(int i=start; i< fin ; i++){
                //pthread_mutex_lock(&mutex);
                u_map[tnum].insert(make_pair(table[left_table_index_in_hash][i][l_key_index],table[left_table_index_in_hash][i]));
                //pthread_mutex_unlock(&mutex);
            }
            ...
            for(int k = 0 ; k< NUM_THREADS; k++){
                for( int i=start ; i<fin ;i++){
                    ...
                    auto its = u_map[k].equal_range(table[right_table_index_in_hash][i][r_key_index]);
...
//    u_map.clear();
    ...
    // for(int i=1; i< table[left_table_index_in_hash].size() ; i++){
