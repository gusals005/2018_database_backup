...
typedef struct trx_t {
    ...
    set<int> wait_transaction_id; // lock object that trx is waiting
...
extern unordered_map<int64_t , vector<lock_t> > lock_table; //make hash_map
extern pthread_mutex_t lock_table_mutex;   //prvent lock_table
extern pthread_mutex_t buffer_pool_mutex;

//Transaction Manager
extern pthread_mutex_t tx_mutex;
extern pthread_cond_t tx_cond;
extern int now_transaction_number;
extern pthread_mutex_t trx_list_lock;  //lock for preventing trx_list
extern vector<trx_t*> trx_list;     // list of running TRX
...
int64_t * find(int table_id, int64_t key, int tid , int* result); // key가 tree에 있는지 확인함 있으면, 그 위치가 return 없으면 null
...
buffer*  buffer_get_page( int table_id ,pagenum_t pagenum, page_t * dest);
