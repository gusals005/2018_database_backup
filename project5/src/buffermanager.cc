...
void buffer_init( buffer* buffer){
    ...
    pthread_mutex_init(&buffer->page_latch, NULL);
    pthread_cond_init(&buffer->page_cond, NULL);
    buffer->owner_tx_id = 0;
    buffer->is_locking = NOT_LOCKING;
}
...
buffer* select_buffer(){        // 빈 버퍼가 있으면, 그 버퍼의 주소가 return 되고, 없으면 NULL이 리턴.
    ...
    while(1){
        ...
        if(iter->pin_count == 0 && iter->is_locking == 0)
            return iter;
        else
...
bool get_page_lock(int table_id, pagenum_t pagenum ){
    buffer* iter = header_buffer;
    while( iter->next_buffer != NULL){
        iter = iter->next_buffer;
        if(iter->table_id == table_id && iter->pagenum == pagenum){
            pthread_mutex_lock(&iter->page_latch);
            return true;
        }
    }
    return false;
